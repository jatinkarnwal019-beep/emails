...
app.get("/", (req, res) => res.send("Outreach backend running OK"));
...
app.post("/send", async (req, res) => {
  try {
    ...
    if (!to || !subject || !body || !fromEmail) return res.status(400).json({ error: "Missing fields" });
    const from = fromName ? fromName + " <" + fromEmail + ">" : fromEmail;
    const html = body.split("\n").map(l => l.trim() ? "<p style='margin:0 0 14px;font-family:sans-serif;font-size:15px;line-height:1.6;color:#1a1a1a'>" + l + "</p>" : "<br>").join("");
